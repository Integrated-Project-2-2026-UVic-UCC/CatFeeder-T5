...
// Zones de cantonada fixes
#define CORNER_HIT_ZONE_PX 70
#define CORNER_LEFT_MIN 0
#define CORNER_LEFT_MAX 70
#define CORNER_RIGHT_MIN 410
#define CORNER_RIGHT_MAX 480
#define CORNER_TOP_MIN 0
#define CORNER_TOP_MAX 70
#define CORNER_BOTTOM_MIN 250
#define CORNER_BOTTOM_MAX 320
...
// Evita comptar el mateix toc diverses vegades
...
static void drawUnlockScreen() {
  ...
  const DrawZone drawZones[4] = {
      {CORNER_LEFT_MIN, CORNER_TOP_MIN, CORNER_HIT_ZONE_PX,
       CORNER_HIT_ZONE_PX}, // TL
      {CORNER_RIGHT_MIN, CORNER_TOP_MIN, CORNER_HIT_ZONE_PX,
       CORNER_HIT_ZONE_PX}, // TR
      {CORNER_LEFT_MIN, CORNER_BOTTOM_MIN, CORNER_HIT_ZONE_PX,
       CORNER_HIT_ZONE_PX}, // BL
      {CORNER_RIGHT_MIN, CORNER_BOTTOM_MIN, CORNER_HIT_ZONE_PX,
       CORNER_HIT_ZONE_PX} // BR
  };
  ...
  for (int i = 0; i < CORNER_COUNT; i++) {
    ...
    tft.fillRect(drawZones[i].x, drawZones[i].y, drawZones[i].w, drawZones[i].h,
                 col);

    tft.drawRect(drawZones[i].x, drawZones[i].y, drawZones[i].w, drawZones[i].h,
                 UI_COL_MUTED);
    ...
    tft.drawString(labels[i], drawZones[i].x + drawZones[i].w / 2,
                   drawZones[i].y + drawZones[i].h / 2);
  }
...
static bool checkCornerTouch(uint16_t tx, uint16_t ty) {
  ...
  const Zone zones[CORNER_COUNT] = {
      {CORNER_LEFT_MIN, CORNER_LEFT_MAX, CORNER_TOP_MIN, CORNER_TOP_MAX}, // TL
      {CORNER_RIGHT_MIN, CORNER_RIGHT_MAX, CORNER_TOP_MIN,
       CORNER_TOP_MAX}, // TR
      {CORNER_LEFT_MIN, CORNER_LEFT_MAX, CORNER_BOTTOM_MIN,
       CORNER_BOTTOM_MAX}, // BL
      {CORNER_RIGHT_MIN, CORNER_RIGHT_MAX, CORNER_BOTTOM_MIN,
       CORNER_BOTTOM_MAX} // BR
  };

  const char *names[CORNER_COUNT] = {"TL", "TR", "BL", "BR"};
  ...
  const Zone &zone = zones[expected];

  Serial.printf("[check] toc px=(%u,%u) esperat=%s zona x=%d..%d y=%d..%d\n",
                tx, ty, names[expected], zone.xMin, zone.xMax, zone.yMin,
                zone.yMax);

  bool inside =
      tx >= zone.xMin && tx <= zone.xMax && ty >= zone.yMin && ty <= zone.yMax;

  if (!inside) {
    Serial.println(F("[unlock] fora de zona"));

    // Si vols que un toc incorrecte reiniciï el patró, descomenta:
    // resetUnlockPattern();
...
static void drawAutoModeScreen() {
  ...
  if (scheduleCount > 0) {
    ...
    tft.setTextColor(UI_COL_MUTED, UI_COL_BG);
    tft.setTextSize(1);
    tft.drawString("Comprova la connexio WiFi i la configuracio a Supabase.",
                   12, iy);
  }
